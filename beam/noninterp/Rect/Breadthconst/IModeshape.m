function [Depth,Lambda]=IModeshape(E,Rho,U,Breadth,L,numele,fixeddofs,Vol,tol)
...
for ele=1:numele    
   ...
   Kvec=E(ele)*Ke*(Breadth(ele)/12)*U(eledofs);
   Mvec=Rho(ele)*Me*Breadth(ele)*U(eledofs);
   for i = 1:4
...
Comul=Breadth*Le;
[Depth,Lambda]=PowerIter(psK,psM,K(freedofs,:),M(freedofs,:),3,Comul,Vol,tol);
end
